    ...
    try:
        with connect(
            ...
            with connection.cursor() as cursor:
                print("SONO IN CONNECTION")
                for index, row in df.iterrows():
                    print(row)
                    query = """INSERT INTO distributori (id, indirizzo, latitudine, longitudine, prezzo_benzina, prezzo_diesel)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    ON DUPLICATE KEY UPDATE
                    indirizzo = VALUES(indirizzo), latitudine = VALUES(latitudine), longitudine = VALUES(longitudine),
                    prezzo_benzina = VALUES(prezzo_benzina), prezzo_diesel = VALUES(prezzo_diesel)
                    """
                    values = (
                        row['id'], row['indirizzo'], row['latitudine'],
                        row['longitudine'], row['prezzo_benzina'],
                        row['prezzo_diesel']
                    )
